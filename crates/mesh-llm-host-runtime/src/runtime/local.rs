...
/// descriptor, so `serving_provenance_for_model` (network/openai/ingress.rs)
/// can attest it on every exchange this node serves for `model_name`. `None`
/// when the file could not be hashed (unreadable) -- left absent, never
/// fabricated. This never touches `identity_hash` (the reference-string hash)
/// -- the two are different facts and neither replaces the other.
