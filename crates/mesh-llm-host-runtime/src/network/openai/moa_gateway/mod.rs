...
/// Per-request routing inputs shared with ordinary single-model routing: the
/// local targets table (for worker-pool assembly), the caller's token
/// budget, cache affinity, and whether `x-mesh-target`/`x-mesh-exclude` were
/// asked to be honored. Grouped so `try_handle_moa` stays under clippy's
/// argument-count lint -- see its doc comment for what
/// `mesh_routing_requested` controls.
pub struct MoaRoutingContext<'a> {
    ...
    pub mesh_routing_requested: bool,
}
...
pub async fn try_handle_moa(
    ...
    let Some(mut config) = admitted_gateway_config(node, targets, required_tokens, affinity).await
    else {
